...
def submit_data():
    ...
    # Check if a new record is created or an existing one is updated
    submission_type = "Updated"
    ...
    if cell:
        ...
        submission_type = "Added"

    # 📝 Log the submission
    log_entry = (
        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] "
        f"{submission_type} - Barcode: {barcode}, "
        f"Location: {location if location else 'N/A'}, "
        f"Quantity Adjusted: {quantity_adjustment}\n"
    )
